...
def executar_interface_nf():
    file_path = CONFIG["heineken_nf"]["file_path"]
    sheet = CONFIG["heineken_nf"]["sheet"]
    header = CONFIG["heineken_nf"].get("header", 1)
    emails_df_path = CONFIG["heineken_nf"]["emails_path"]
    ...
    try:
        ...
        df = df[df["data entrega"].notna() & df["data entrada"].isna()]
    ...
    df["base/cidade"] = df["base"].astype(str) + " - " + df["depositantes"].astype(str)
    ...
    def enviar_emails():
        ...
        for base, group in agrupado:
            ...
            for _, row in group.iterrows():
                corpo_lista += f"- Pedido {row['pedido']} – NF {row['nf']} – Entregue em {row['data entrega'].strftime('%d/%m/%Y')}\n"
...
"""
    ...
    root = tk.Tk()
    root.title("Cobrança de NF – Heineken")
