...
def executar_interface_nf():
    file_path = CONFIG["lg_nf"]["file_path"]
    sheet = CONFIG["lg_nf"]["sheet"]
    header = CONFIG["lg_nf"].get("header", 1)
    emails_df_path = CONFIG["lg_nf"]["emails_path"]
    ...
    df.rename(columns={"base": "base", "cidade": "cidade", "nf": "n° nf", "pedido": "n° pedido"}, inplace=True)
    ...
    df["base/cidade"] = df["base"].astype(str) + " - " + df["cidade"].astype(str)
    ...
    def enviar_emails():
        ...
        for base, group in agrupado:
            ...
            for _, row in group.iterrows():
                corpo_lista += f"- Pedido {row['n° pedido']} – NF {row['n° nf']} – Entregue em {row['data da entrega'].strftime('%d/%m/%Y')}\n"
...
"""
        ...
        if log_linhas:
            ...
            historico_data = [{
                ...
                "Pedido": row["n° pedido"],
                "NF": row["n° nf"],
                "Data entrega": row["data da entrega"],
    ...
    def preencher_text_area(filtro=""):
        ...
        for base, group in agrupado:
            ...
            for _, row in group.iterrows():
                ...
                text_area.insert(tk.END, f" | PEDIDO: {row['n° pedido']}")
