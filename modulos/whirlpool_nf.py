...
def executar_interface_nf():
    file_path = CONFIG["whirlpool_nf"]["file_path"]
    sheet = CONFIG["whirlpool_nf"]["sheet"]
    header = CONFIG["whirlpool_nf"].get("header", 1)
    emails_df_path = CONFIG["whirlpool_nf"]["emails_path"]
    ...
    try:
        ...
        df["data da entrega"] = pd.to_datetime(df["data da entrega"], errors="coerce", dayfirst=True)
...
"""
    ...
    root = tk.Tk()
    root.title("Cobrança de NF – Whirlpool")
