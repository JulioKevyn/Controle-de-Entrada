...
def executar_interface_nf():
    file_path = CONFIG["opella_nf"]["file_path"]
    sheet = CONFIG["opella_nf"]["sheet"]
    header = CONFIG["opella_nf"].get("header", 1)
    emails_df_path = CONFIG["opella_nf"]["emails_path"]
...
"""
    ...
    root = tk.Tk()
    root.title("Cobrança de NF – Opella")
