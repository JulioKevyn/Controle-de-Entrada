...
def executar_interface_nf():
    file_path = CONFIG["diageo_nf"]["file_path"]
    sheet = CONFIG["diageo_nf"]["sheet"]
    header = CONFIG["diageo_nf"].get("header", 1)
    emails_df_path = CONFIG["diageo_nf"]["emails_path"]
    ...
    try:
        ...
        df = df[df["data de entrega na base"].notna() & df["data de entrada no site"].isna()]
        df["data de entrega na base"] = pd.to_datetime(df["data de entrega na base"], errors="coerce") 
    ...
    df.rename(columns={
        "data de entrada no site": "data de entrada no site",
        "data classificacao": "data classificacao",
        "data de entrega na base": "data de entrega na base"
    }, inplace=True)
    ...
    df["base/cidade"] = df["base"].astype(str) + " - " + df["depositante"].astype(str)
    ...
    def enviar_emails():
        ...
        for base, group in agrupado:
            ...
            for _, row in group.iterrows():
                entrega = row["data de entrega na base"]
                entrega_formatada = entrega.strftime('%d/%m/%Y') if pd.notna(entrega) else "Data inválida"
                corpo_lista += f"- Pedido {row['pedido']} – NF {row['nf']} – Entregue em {entrega_formatada}\n"
...
"""
        ...
        if log_linhas:
            ...
            historico_data = [{
                ...
                "Pedido": row["pedido"],
                "NF": row["nf"],
                "Data de entrega na Base": row["data de entrega na base"],
    ...
    root = tk.Tk()
    root.title("Cobrança de NF – Diageo")
    ...
    def preencher_text_area(filtro=""):
        ...
        for base, group in agrupado:
            ...
            for _, row in group.iterrows():
                ...
                entrega = row["data de entrega na base"]
                entrega_formatada = entrega.strftime('%d/%m/%Y') if pd.notna(entrega) else "Data inválida"
                text_area.insert(tk.END, f" | ENTREGA: {entrega_formatada} | EMAILS: {emails if emails else 'Não encontrado'}\n")
