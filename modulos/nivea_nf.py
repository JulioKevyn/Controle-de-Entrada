...
def executar_interface_nf():
    file_path = CONFIG["nivea_nf"]["file_path"]
    sheet = CONFIG["nivea_nf"]["sheet"]
    header = CONFIG["nivea_nf"].get("header", 1)
    emails_df_path = CONFIG["nivea_nf"]["emails_path"]
    ...
    try:
        ...
        df = df[df["data de entrada base"].notna() & df["data de entrada site"].isna()]
        df["data de entrada base"] = pd.to_datetime(df["data de entrada base"], errors="coerce", dayfirst=True)  
    ...
    df.rename(columns={
        "data de entrada site": "data de entrada site",
        "data classificacao": "data classificacao",
        "data de entrada base": "data de entrada base"
    }, inplace=True)
    ...
    def enviar_emails():
        ...
        for base, group in agrupado:
            ...
            for _, row in group.iterrows():
                entrega = row["data de entrada base"]
...
"""
        ...
        if log_linhas:
            ...
            historico_data = [{
                ...
                "Pedido": row["pedido"],
                "NF": row["nf"],
                "data de entrada base": row["data de entrada base"],
    ...
    root = tk.Tk()
    root.title("Cobrança de NF – Nivea")
