...
def executar_interface_nf():
    file_path = CONFIG["mondelez_nf"]["file_path"]
    sheet = CONFIG["mondelez_nf"]["sheet"]
    header = CONFIG["mondelez_nf"].get("header", 1)
    emails_df_path = CONFIG["mondelez_nf"]["emails_path"]
    ...
    try:
        ...
        df = df[df["entrega realizada"].notna() & df["data entrada sistema"].isna()]
    ...
    df.rename(columns={
        "os": "n° os",
        "nf": "n° nf",
        "data entrada sistema": "data entrada",
        "data classificacao": "data classificacao",
        "entrega realizada": "data entrega"
    }, inplace=True)
    ...
    df["base/cidade"] = df["base"].astype(str) + " - " + df["cliente"].astype(str)
    ...
    def enviar_emails():
        ...
        for base, group in agrupado:
            ...
            for _, row in group.iterrows():
                corpo_lista += f"- Pedido {row['n° os']} – NF {row['n° nf']} – Entregue em {row['data entrega'].strftime('%d/%m/%Y')}\n"
...
"""
    ...
    root = tk.Tk()
    root.title("Cobrança de NF – Mondelez")
    ...
    def preencher_text_area(filtro=""):
        ...
        for base, group in agrupado:
            ...
            for _, row in group.iterrows():
                ...
                text_area.insert(tk.END, f" | PEDIDO: {row['n° os']}")
                text_area.insert(tk.END, f" | NF: {row['n° nf']}")
                text_area.insert(tk.END, f" | ENTREGA: {row['data entrega'].strftime('%d/%m/%Y')} | EMAILS: {emails if emails else 'Não encontrado'}\n")
